...
this.Server.prototype.serveDir = function (pathname, req, res, finish) {
    ...
    fs.stat(htmlIndex, function (e, stat) {
        if (!e) {
            that.respond(200, {}, [htmlIndex], stat, req, res, finish);
        } else {
    ...
    function streamFiles(files) {
        var out = []
        for(var i = files.length-1; i>=0; --i) { out.push(path.join(pathname, files[i])) } 
        util.mstat(out, function (e, stat) {
            if (!e) {
              that.respond(200, {}, out, stat, req, res, finish);
            }
        });
    }
};
...
this.Server.prototype.respond = function (status, _headers, files, stat, req, res, finish) {
    ...
    if (req.headers['if-none-match'] === headers['Etag'] &&
        ...
        if (this.cache && (key in exports.store) &&
            ...
            this.stream(files, new(buffer.Buffer)(stat.size), res, function (e, buffer) {
...
this.Server.prototype.stream = function (files, buffer, res, callback) {
